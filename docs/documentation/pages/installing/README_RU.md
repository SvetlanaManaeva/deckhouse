...
* Установка Deckhouse.
* Создание, настройка и удаление узлов кластера Kubernetes.
* Поддержание (приведение) кластера в состояние, описанное в конфигурации.

Варианты установки Deckhouse:
- **в поддерживаемых облаках** — dhctl автоматически создает и настраивает все необходимые ресурсы (включая виртуальные машины), разворачивает кластер Kubernetes, устанавливает Deckhouse. Поддерживаемые облачные инфраструктуры и провайдеры можно найти в разделе [Кластер Kubernetes](../kubernetes.html) документации.
- в кластерах **bare metal** и **в неподдерживаемых облаках** — dhctl настраивает сервер (виртуальную машину), разворачивает кластер Kubernetes с одним master-узлом и устанавливает Deckhouse. Далее, с помощью готовых скриптов настройки, можно вручную добавить дополнительные узлы в кластер.
- **в существующем кластере Kubernetes** — dhctl устанавливает Deckhouse.

## Подготовка инфраструктуры

Перед установкой проверьте следующее:
- *(для кластеров на bare metal и в неподдерживаемых облаках)* ОС сервера находится в [списке поддерживаемых ОС](../supported_versions.html) (или совместима с ними) и до сервера есть SSH-доступ;
...
Запуск контейнера инсталлятора из container registry Deckhouse в общем случае выглядит так:
...
- `<MOUNT_OPTIONS>` — параметры монтирования файлов в контейнер инсталлятора, такие как:
  ...
  - файл конфигурации kubectl (для установки в существующем кластере)
...
Для получения справки по параметрам выполните `dhctl help bootstrap`.

Пример запуска установки:

```shell
dhctl bootstrap \
  --ssh-user=ubuntu --ssh-agent-private-keys=/tmp/.ssh/id_rsa \
...
- `ubuntu` — пользователь на сервере для подключения по SSH;
