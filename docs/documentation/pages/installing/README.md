...
The page is under active development and may contain incomplete information.
</div>

To install Deckhouse, we recommend using the [Getting Started](/en/gs/) guide, where you may find step-by-step instructions for deploying the platform.

!!!!!!!!!!! If necessary, read the description of the [resources](configuration.html) used when deploying a cluster or installing Deckhouse in an existing cluster.
